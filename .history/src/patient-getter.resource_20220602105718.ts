...
export async function getPatient(query) {
  ...
  const getValue = (object) =>(typeof object == "undefined" ? "": object)
  ...
  if (searchResult?.data?.entry) {
    searchResult?.data?.entry.forEach(function (item, i) {
      ...
      patients.push({
        ...
        residence: (item?.resource?.address).length > 0? getValue(item.resource.address[0]?.country),
      //   //   (item?.resource?.address).length > 0 ? (item.resource.address[0].country +
      //   //   ", " +
